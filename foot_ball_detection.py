...
#Import libraries
import cv2
import numpy as np#Reading the video
...
while success:
	...
	upper_blue = np.array([130,255,255])#Red range
	lower_red = np.array([0,31,255])
	upper_red = np.array([176,255,255])#white range
...
for c in contours:
  ...
  if(h>=(1.5)*w):
   if(w>15 and h>= 15):
    ...
    nzCountred = cv2.countNonZero(res2)if(nzCount >= 20):
